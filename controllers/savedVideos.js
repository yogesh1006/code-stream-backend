...
module.exports = {
  ...
  removeSavedVideo: async (req, res) => {
    try {
       const deletedData = await User.findOneAndUpdate({ username: req.body.username }, { $pull: { savedVideos: req.body.id } },{new:true});
            res.json({
                status: 'success',
        ...
        res.status(400).json({
            message: (error && error.message) || 'Oops! Failed to remove saved video.'
        })
    }
